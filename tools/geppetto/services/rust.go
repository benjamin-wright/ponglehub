...
func (r Rust) startBuilder(ctx context.Context, repo types.Repo) error {
	output, err := r.cmd.Run(
		...
		repo.Path,
		fmt.Sprintf(
			"docker run -d --name %s-builder -v $(pwd):/volume:delegated -v %s-cargo-git:/root/.cargo/git -v %s-cargo-registry:/root/.cargo/registry -v %s-cargo-target:/volume/target clux/muslrust:nightly tail -f /dev/null",
			repo.Name,
			repo.Name,
			repo.Name,
			repo.Name,
